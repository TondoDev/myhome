...
//	ShareSummary getSumOfPaymentsAndFees(Fond fond);
	...
	@Query("select f.parentFond.id as fondId, sum(f.fee) as totalFees, sum(f.buyPrice/f.unitPrice) as ownedUnitsCount, sum(f.buyPrice) as totalBuyPrice from #{#entityName} f where parentFond = ?1 group by parentFond")
	ShareSummary getSumOfPaymentsAndFees(Fond fond);
